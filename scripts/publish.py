...
def check_build_files():
    ...
    if not dist_dir.exists():
        print(
            "❌ dist/ ディレクトリが見つかりません。先にビルドを実行してください。"
        )
        return False
    ...
    if not files:
        print(
            "❌ ビルドファイルが見つかりません。先にビルドを実行してください。"
        )
        return False
...
def test_upload():
    ...
    if not run_command(
        "twine upload --repository testpypi dist/*", "TestPyPIアップロード"
    ):
        return False

    print("\n✅ TestPyPIアップロードが完了しました")
    print(
        "   テスト用URL: https://test.pypi.org/project/github-actions-ai-analyzer/"
    )
    return True
...
def production_upload():
    ...
    print("\n🎉 PyPIアップロードが完了しました！")
    print(
        "   パッケージURL: https://pypi.org/project/github-actions-ai-analyzer/"
    )
    return True
...
def main():
    ...
    print("\n📋 次のステップ:")
    print(
        "1. PyPIでパッケージを確認: https://pypi.org/project/github-actions-ai-analyzer/"
    )
