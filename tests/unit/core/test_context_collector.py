...
class TestContextCollector:
    ...
    def test_init(self):
        """初期化のテスト"""
        assert self.collector.supported_languages == {
    ...
    def test_detect_language_and_frameworks_python(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            language, frameworks = self.collector._detect_language_and_frameworks(temp_path)
    ...
    def test_detect_package_managers(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Python
            (temp_path / "requirements.txt").write_text("requests==2.28.0")
            package_managers = self.collector._detect_package_managers(temp_path)
...
pytest==7.0.0
            ...
            dependencies = self.collector._collect_dependencies(temp_path, "python")
    ...
    def test_collect_dependencies_javascript(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dependencies = self.collector._collect_dependencies(temp_path, "javascript")
    ...
    def test_collect_dependencies_unknown_language(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dependencies = self.collector._collect_dependencies(temp_path, None)
            assert dependencies == {}
    ...
    def test_find_workflow_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('pathlib.Path') as mock_path:
                ...
                # 実際のファイルシステムに依存するため、結果は不定
                assert result is None or result.endswith("ci.yml")
    ...
    def test_parse_workflow_yaml(self):
        ...
        context = self.collector._parse_workflow_yaml(workflow_data, "/path/to/workflow.yml")
    ...
    def test_detect_available_tools(self, mock_run):
        ...
        # 基本的なツールが含まれているかチェック
        expected_tools = ["git", "python", "node", "npm", "java", "mvn", "gradle"]
