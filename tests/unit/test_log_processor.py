...
"""
        ...
        assert len(result) == 4
        assert result[0].message == "Step 1: Install dependencies"
        assert result[1].level == LogLevel.ERROR
        assert result[1].message == "ModuleNotFoundError: No module named 'requests'"
        assert result[2].message == "Step 2: Run tests"
        assert result[3].level == LogLevel.WARNING
        assert result[3].message == "Deprecated feature used"
    
    def test_determine_log_level(self):
        """ログレベルの判定をテスト"""
        assert self.processor._determine_log_level("error: something went wrong") == LogLevel.ERROR
        assert self.processor._determine_log_level("Error: something went wrong") == LogLevel.ERROR
        assert self.processor._determine_log_level("ERROR: something went wrong") == LogLevel.ERROR
        assert self.processor._determine_log_level("warning: deprecated") == LogLevel.WARNING
        assert self.processor._determine_log_level("info: information") == LogLevel.INFO
        assert self.processor._determine_log_level("normal message") == LogLevel.INFO
    ...
    def test_filter_by_level(self):
        ...
        assert all(entry.level in [LogLevel.WARNING, LogLevel.ERROR] for entry in filtered)
