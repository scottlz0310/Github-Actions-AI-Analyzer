...
class AIPromptOptimizer:
    ...
    def generate_error_analysis_prompt(self, analysis_result: AnalysisResult) -> str:
        """エラー解析用のプロンプトを生成"""
        template = self.templates['error_analysis']
        
        # エラー情報を構造化
        error_summary = self._format_error_summary(analysis_result.error_analyses)
    ...
    def generate_solution_prompt(self, analysis_result: AnalysisResult) -> str:
        ...
        # エラー詳細を構造化
        error_details = self._format_error_details(analysis_result.error_analyses)
        ...
        return template.format(
            ...
            repository_language=analysis_result.repository_context.language or "unknown",
        )
    
    def generate_best_practices_prompt(self, analysis_result: AnalysisResult) -> str:
        """ベストプラクティス提案用のプロンプトを生成"""
        template = self.templates['best_practices']
        
        # 現在の設定を構造化
        current_setup = self._format_current_setup(analysis_result)
        error_patterns = self._format_error_patterns(analysis_result.error_analyses)
    ...
    def _format_error_summary(self, error_analyses: List[ErrorAnalysis]) -> str:
        """エラーサマリーをフォーマット"""
        ...
        for analysis in error_analyses:
            ...
            if analysis.affected_steps:
                summary_parts.append(f"  影響ステップ: {', '.join(analysis.affected_steps)}")
            if analysis.related_files:
                summary_parts.append(f"  関連ファイル: {', '.join(analysis.related_files)}")
    ...
    def _format_error_details(self, error_analyses: List[ErrorAnalysis]) -> str:
        """エラー詳細をフォーマット"""
        if not error_analyses:
            return "エラーは検出されませんでした。"
        ...
        for analysis in error_analyses:
            ...
            if analysis.pattern_matches:
                ...
                for match in analysis.pattern_matches:
                    details_parts.append(f"- {match.pattern.name}: {match.matched_text}")
            
            details_parts.append("")  # 空行
        
        return "\n".join(details_parts)
    
    def _format_existing_solutions(self, solutions: List[SolutionProposal]) -> str:
        """既存の解決策をフォーマット"""
        if not solutions:
            return "既存の解決策はありません。"
    ...
    def _format_error_patterns(self, error_analyses: List[ErrorAnalysis]) -> str:
        """エラーパターンをフォーマット"""
        if not error_analyses:
