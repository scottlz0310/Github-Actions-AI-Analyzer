...
from ..types import RepositoryContext, WorkflowContext, EnvironmentContext
...
class ContextCollector:
    ...
    def collect_repository_context(self, repository_path: Optional[str] = None) -> RepositoryContext:
        """リポジトリのコンテキスト情報を収集"""
    ...
    def collect_workflow_context(self, workflow_file_path: Optional[str] = None) -> WorkflowContext:
        """ワークフローのコンテキスト情報を収集"""
    ...
    def _detect_language_and_frameworks(self, repo_path: Path) -> tuple[Optional[str], List[str]]:
        """言語とフレームワークを検出"""
        frameworks = []
    ...
    def _collect_dependencies(self, repo_path: Path, language: Optional[str]) -> Dict[str, Any]:
        """依存関係情報を収集"""
        dependencies = {}
        ...
        if language == 'python':
            ...
            if package_json.exists():
                try:
                    ...
                    dependencies['devDependencies'] = data.get('devDependencies', {})
                except:
                    pass
    ...
    def _parse_workflow_yaml(self, workflow_data: Dict[str, Any], file_path: str) -> WorkflowContext:
        """ワークフローYAMLを解析"""
