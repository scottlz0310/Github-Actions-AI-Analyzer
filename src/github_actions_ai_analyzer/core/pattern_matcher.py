...
from ..types import ErrorPattern, PatternMatch, PatternCategory, LogEntry
...
class PatternMatcher:
    ...
    def match_patterns(self, log_entries: List[LogEntry]) -> List[PatternMatch]:
    ...
    def _match_entry(self, entry: LogEntry) -> List[PatternMatch]:
        ...
        for pattern in self.patterns:
            # 言語フィルタリング
            if pattern.language and entry.metadata.get('language') != pattern.language:
                continue
    ...
    def _calculate_confidence(self, pattern: ErrorPattern, entry: LogEntry, match: re.Match) -> float:
        """マッチングの信頼度を計算"""
        confidence = 0.5  # ベース信頼度
        
        # ログレベルによる調整
        if entry.level.value == "error" and pattern.severity == "error":
            confidence += 0.2
        elif entry.level.value == "warning" and pattern.severity == "warning":
            confidence += 0.1
    ...
    def get_patterns_by_category(self, category: PatternCategory) -> List[ErrorPattern]:
        """カテゴリ別にパターンを取得"""
