...
@click.option('--repository', '-r', type=click.Path(exists=True), 
              help='リポジトリのパス')
@click.option('--min-level', '-l', type=click.Choice(['debug', 'info', 'warning', 'error', 'fatal']),
              default='warning', help='最小ログレベル')
...
def analyze(log_file, workflow, repository, min_level, output):
    """ログファイルを解析してエラー分析を実行"""
    try:
        console.print(f"[bold blue]GitHub Actions AI Analyzer[/bold blue]")
...
def validate(workflow_file):
    """ワークフローファイルを検証"""
    console.print(f"[bold blue]ワークフローファイル検証[/bold blue]")
...
def watch(directory, interval):
    ...
    console.print(f"[bold blue]ログファイル監視[/bold blue]")
    ...
    if result.error_analyses:
        ...
        for analysis in result.error_analyses:
            error_table.add_row(
                ...
                analysis.root_cause,
                analysis.severity,
                ", ".join(analysis.affected_steps) if analysis.affected_steps else "なし"
            )
...
def _display_yaml_result(result):
    """YAML形式で結果を表示"""
    import yaml
    console.print(yaml.dump(result.model_dump(), default_flow_style=False, allow_unicode=True))
