...
class RepositoryContext(BaseModel):
    ...
    frameworks: List[str] = Field(default_factory=list, description="使用フレームワーク")
    package_managers: List[str] = Field(default_factory=list, description="パッケージマネージャー")
    dependencies: Dict[str, Any] = Field(default_factory=dict, description="依存関係情報")
...
class WorkflowContext(BaseModel):
    ...
    steps: List[str] = Field(default_factory=list, description="ステップ名リスト")
    actions: List[str] = Field(default_factory=list, description="使用アクションリスト")
...
class EnvironmentContext(BaseModel):
    ...
    available_tools: List[str] = Field(default_factory=list, description="利用可能ツール")
    environment_variables: Dict[str, str] = Field(default_factory=dict, description="環境変数")
