...
class ErrorAnalysis(BaseModel):
    ...
    pattern_matches: List[PatternMatch] = Field(..., description="マッチしたパターン")
    root_cause: str = Field(..., description="推定される根本原因")
    severity: str = Field(..., description="重要度")
    affected_steps: List[str] = Field(default_factory=list, description="影響を受けるステップ")
    related_files: List[str] = Field(default_factory=list, description="関連ファイル")
...
class SolutionProposal(BaseModel):
    ...
    code_examples: List[str] = Field(default_factory=list, description="コード例")
...
class AnalysisResult(BaseModel):
    ...
    repository_context: RepositoryContext = Field(..., description="リポジトリコンテキスト")
    workflow_context: WorkflowContext = Field(..., description="ワークフローコンテキスト")
    environment_context: EnvironmentContext = Field(..., description="環境コンテキスト")
    error_analyses: List[ErrorAnalysis] = Field(..., description="エラー解析結果")
    solution_proposals: List[SolutionProposal] = Field(..., description="解決策提案")
