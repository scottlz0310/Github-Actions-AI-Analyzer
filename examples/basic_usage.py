...
def main():
    ...
    try:
        ...
        for i, error in enumerate(result.error_analyses, 1):
            ...
            print(f"  影響ステップ: {', '.join(error.affected_steps) if error.affected_steps else 'なし'}")
            print(f"  関連ファイル: {', '.join(error.related_files) if error.related_files else 'なし'}")
        ...
        print("\n解決策生成用プロンプト:")
        print("-" * 30)
        print(solution_prompt[:500] + "..." if len(solution_prompt) > 500 else solution_prompt)
